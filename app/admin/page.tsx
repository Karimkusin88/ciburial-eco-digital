...
export default function AdminPage() {
  ...
  const addKegiatan = async () => {
    ...
    setLoading(true);
    const { error } = await supabase.from("kegiatan").insert({ ...kForm, foto: kForm.foto || null });
    setLoading(false);
    if (error) return showToast("❌ Gagal menyimpan: " + error.message);
    setKForm(emptyK); fetchAll(); showToast("✅ Kegiatan berhasil ditambahkan!");
  };
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:20, alignItems:"start" }}>
            ...
            <div style={{ background:"#FFFEF9", border:"1px solid #E5E0D8", borderRadius:20, padding:"28px" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <div>
                  <label style={{ display:"block", fontSize:10, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:"#9A8C85", marginBottom:6 }}>URL Foto (opsional)</label>
                  <input className="field" value={kForm.foto} onChange={e => setKForm({...kForm, foto:e.target.value})} placeholder="https://..." />
                  <p style={{ fontSize:11, color:"#9A8C85", marginTop:5 }}>Upload foto ke Google Drive / ImgBB, paste URL-nya di sini</p>
                </div>
