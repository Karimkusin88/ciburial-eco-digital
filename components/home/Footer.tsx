      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,.05)", padding: "clamp(12px, 3vw, 20px) clamp(12px, 3vw, 24px)", maxWidth: 1320, margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 400, color: "rgba(250,248,243,.55)", lineHeight: 1.4 }}>
            Built by <strong style={{ color: "var(--cw)", fontWeight: 700 }}>Ubay Rahmat</strong>
          </div>
        </div>

        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <a href="mailto:ubayrahmat@iys.works" aria-label="Email" style={{ color: "rgba(250,248,243,.45)", transition: "all .2s", transform: "translateY(0)", display: "inline-flex" }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.color = "var(--cr)"; (e.currentTarget as HTMLAnchorElement).style.transform = "translateY(-2px)" }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.color = "rgba(250,248,243,.45)"; (e.currentTarget as HTMLAnchorElement).style.transform = "translateY(0)" }}>
            <svg style={{ width: SOCIAL_SIZE, height: SOCIAL_SIZE }} fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4.236l-8 4.882-8-4.882V6.309l8 4.882 8-4.882v1.927z"/></svg>
          </a>
          <a href="https://www.linkedin.com/in/ubay-rahmat-hidayat-2b4b1431b/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" style={{ color: "rgba(250,248,243,.45)", transition: "all .2s", transform: "translateY(0)", display: "inline-flex" }} onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.color = "var(--cr)"; (e.currentTarget as HTMLAnchorElement).style.transform = "translateY(-2px)" }} onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.color = "rgba(250,248,243,.45)"; (e.currentTarget as HTMLAnchorElement).style.transform = "translateY(0)" }}>
            <svg style={{ width: SOCIAL_SIZE, height: SOCIAL_SIZE }} fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </a>
